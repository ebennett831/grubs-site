...
function BulkPhotoImportComponent({tool}: {tool: Tool}) {
  ...
  const selectedImageFiles = useMemo(() => selectedFiles.filter(isImageFile), [selectedFiles]);

  const loadGalleries = useCallback(async () => {
    try {
      const results = await client.fetch<GalleryOption[]>(
        `*[_type == "gallery"] | order(coalesce(sortOrder, 0) desc, _createdAt desc){_id, title}`,
      );
  ...
  async function importFiles(files: File[]) {
    ...
    try {
      ...
      const destination = selectedGalleryId
        ? galleries.find((gallery) => gallery._id === selectedGalleryId)?.title || "selected gallery"
        : "ungrouped";
      ...
      <div style={{display: "grid", gap: 24, maxWidth: 1100}}>
        <div style={{display: "grid", gap: 8}}>
          ...
          <p style={{margin: 0, color: "var(--card-fg-color, #6b7280)"}}>
            Choose photo files, confirm the import, and the tool will create photo documents for you in the background.
          </p>
        </div>
        ...
        >
          <div style={{display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "center"}}>
            <div style={{display: "grid", gap: 4}}>
              <h3 style={{margin: 0, fontSize: 18, fontWeight: 700}}>Import settings</h3>
          ...
          <label style={{display: "grid", gap: 8}}>
            <span style={{fontSize: 14, fontWeight: 700}}>Add to gallery</span>
            <select
        ...
        >
          <div style={{display: "grid", gap: 8}}>
            ...
            <p style={{margin: 0}}>1. Click the large button below and choose one or more image files.</p>
            <p style={{margin: 0}}>2. Review the selected files, then click Import selected photos.</p>
            <p style={{margin: 0}}>3. The tool uploads each file, creates the photo document, and uses the file name as alt text.</p>
          </div>
          ...
          >
            ...
            />
            <div style={{display: "grid", gap: 8}}>
              <span style={{fontSize: 18, fontWeight: 700}}>{isImporting ? "Importing files..." : "Choose photo files"}</span>
              <span style={{fontSize: 14, color: "var(--card-fg-color, #6b7280)"}}>
                Press this button to pick images from your computer.
              </span>
            </div>
            <span style={{fontSize: 12, color: "var(--card-fg-color, #6b7280)"}}>
          ...
          <div style={{display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap"}}>
            ...
            {selectedImageFiles.length > 0 ? (
              ...
              <span style={{fontSize: 13, color: "var(--card-fg-color, #6b7280)"}}>No files selected yet.</span>
            )}
          </div>
