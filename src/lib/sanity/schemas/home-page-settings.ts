...
export const homePageSettingsSchema = defineType({
  ...
  fields: [
    ...
    defineField({ name: "heroDescription", title: "Hero Description", type: "text", rows: 5, validation: (rule) => rule.required() }),
    defineField({ name: "heroImage", title: "Hero Image", type: "image", options: { hotspot: true } }),
    defineField({ name: "sectionTitle", title: "Photography Section Title", type: "string", validation: (rule) => rule.required() }),
    defineField({ name: "sectionDescription", title: "Photography Section Description", type: "text", rows: 5, validation: (rule) => rule.required() }),
