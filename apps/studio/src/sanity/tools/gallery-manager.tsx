...
function GalleryManagerComponent({ tool }: { tool: Tool }) {
  ...
  async function updateSortOrder(index: number, direction: "up" | "down") {
    ...
    const target =
      direction === "up" ? galleries[index - 1] : galleries[index + 1];
    ...
    setSuccess(
      `Success: moved \"${current.title}\" ${direction === "up" ? "up" : "down"}.`,
    );
    await refresh();
  }
            ...
            {galleries.length ? (
              galleries.map((gallery, index) => {
                ...
                const imageUrl = coverImage?.asset?._ref
                  ? urlFor(coverImage).width(240).auto("format").url()
                  : "";
                const coverLabel = gallery.coverPhoto
                  ? "Has cover"
                  : "No cover yet";
