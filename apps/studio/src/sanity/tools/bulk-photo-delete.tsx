...
function BulkPhotoDeleteComponent({tool}: {tool: Tool}) {
    ...
    async function loadData() {
      ...
      try {
        const galleryResults = await client.fetch<GalleryOption[]>(
          `*[_type == "gallery"] | order(sortOrder desc, _createdAt desc){_id, title}`,
        );
  ...
  const allVisibleSelected = useMemo(
    () => photos.length > 0 && photos.every((photo) => selectedIds.includes(photo._id)),
    [photos, selectedIds],
  );
  ...
  function toggleAllVisible() {
    setSelectedIds((current) => {
      if (photos.every((photo) => current.includes(photo._id))) {
        return current.filter((id) => !photos.some((photo) => photo._id === id));
      }

      return Array.from(new Set([...current, ...photos.map((photo) => photo._id)]));
    });
  }
  ...
  async function handleDelete(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus(`Deleted ${selectedIds.length} photo document${selectedIds.length === 1 ? "" : "s"}.`);
      setSuccess(`Success: deleted ${selectedIds.length} photo document${selectedIds.length === 1 ? "" : "s"}.`);
      ...
      <div style={{display: "grid", gap: 24, maxWidth: 1200}}>
        <div style={{display: "grid", gap: 8}}>
          ...
          <p style={{margin: 0, color: "var(--card-fg-color, #6b7280)"}}>
            Pick a gallery or ungrouped photos, select the ones you want gone, and delete them in one batch.
          </p>
        </div>
        ...
        >
          <div style={{display: "grid", gap: 8, gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))"}}>
            ...
            <div style={{padding: 12, borderRadius: 12, background: "rgba(17,24,39,0.04)"}}>
              ...
              <p style={{margin: "6px 0 0", fontSize: 20, fontWeight: 700}}>{selectedCount}</p>
            </div>
          </div>
          ...
          <form onSubmit={handleDelete} style={{display: "grid", gap: 12}}>
            <div style={{display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap"}}>
              <button
                ...
                style={{
                  ...
                  background: "white",
                  fontWeight: 700,
                  cursor: !photos.length || isLoading ? "not-allowed" : "pointer",
                }}
              >
              ...
              <button
                ...
                style={{
                  ...
                  color: "white",
                  fontWeight: 700,
                  cursor: !selectedCount || isDeleting ? "not-allowed" : "pointer",
                }}
              >
              ...
              <p style={{margin: 0, fontSize: 13, color: "var(--card-fg-color, #6b7280)"}}>
                Deleting removes the photo documents from Studio. Uploaded image assets are not removed.
              </p>
            </div>
            ...
            {photos.length ? (
              ...
              >
                {photos.map((photo) => {
                  const imageUrl = photo.image?.asset?._ref ? urlFor(photo.image).width(600).auto("format").url() : "";
                  ...
                  return (
                    <label
                      ...
                      style={{
                        ...
                        padding: 12,
                        cursor: "pointer",
                        background: selectedIds.includes(photo._id) ? "rgba(17,24,39,0.04)" : "white",
                        boxShadow: selectedIds.includes(photo._id) ? "0 0 0 1px rgba(17,24,39,0.08)" : "none",
                      }}
                    >
                      <input
                        type="checkbox"
                      ...
                      <div style={{display: "grid", gap: 4}}>
                        <span style={{fontSize: 14, fontWeight: 700, lineHeight: 1.4}}>{photo.altText}</span>
                        {photo.gallery?.title ? (
                          <span style={{fontSize: 12, color: "var(--card-fg-color, #6b7280)"}}>{photo.gallery.title}</span>
                        ) : (
                          <span style={{fontSize: 12, color: "var(--card-fg-color, #6b7280)"}}>Ungrouped</span>
                        )}
                      </div>
                    </label>
                  );
              ...
              <p style={{margin: 0, color: "var(--card-fg-color, #6b7280)"}}>
                {isLoading ? "Loading photos..." : "No photos match this filter."}
              </p>
            )}
          </form>
        </section>
